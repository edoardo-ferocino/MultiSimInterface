function [varargout]=multiFigureBiomed(P,nP,Var,F,iF,Type,Root,SaveFig)
for iP=1:nP
    if(P(iP).Order>0)
...
for iw=1:nw
    h(iw)=FFS('Name',[F(iF).Label '_' num2str(iw) '_' Type],'Position',[0 0 scrsz(3) scrsz(4)]);
    h(iw).ToolBar = 'none'; h(iw).MenuBar = 'none';
    subplot1(nr,nc, 'XTickL', 'Margin', 'YTickL', 'Margin','YScale','linear','FontS',12);
    %subplot1(nr,nc,'Gap',[0.025 0.025],'Min',[0.07 0.09],'Max',[1.01 0.98],'XTickL', 'Margin', 'YTickL', 'Margin','YScale','linear','FontS',22);
    for ir=1:nr
        for ic=1:nc
            subplot1(ic+nc*(ir-1));
            ...
            if Type=='C'
                ...
                PH=pcolor(xx,yy,zvalues); hold on; shading interp; colormap('jet');
                [C,ch]=contour(xx,yy,zvalues,F(iF).Levels,'b','ShowText','on'); hold on;
                clabel(C,ch,'FontSize',14);
                contour(xx,yy,zvalues,[F(iF).Treshold 1E30],'k','LineWidth',2);
                rectangle('Position',[max(PH.Parent.XLim(1),-15) max(PH.Parent.YLim(1),-10) 30 20],'LineWidth',2);
            ...
            if(ir==nr), xlabel([lx ' (' ux ')']);
                %AH=gca; AH.FontSize = 12;
            end
            if nV == 2
                ...
                if nV == 3
                    ...
                    if(ic==1), ylabel({[lr ' = ' num2str(P(or).Range(ir)) ' ' ur];[ly ' (' uy ')']});
                    %    AH=gca; AH.FontSize = 12;
                    end
                    if(ir==1)
                        if iscell(P(oc).Range(ic))
                        title([lc ' = ' num2str(P(oc).Range{ic}) ' ' uc]); 
                        else
                        title([lc ' = ' num2str(P(oc).Range(ic)) ' ' uc]); 
                        end
                        %AH=gca; AH.FontSize = 12;
